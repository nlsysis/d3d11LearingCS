...
LightingApp::LightingApp(HINSTANCE hInstance)
	...
	mVertexShader(0), mPixelShader(0), mWireframeRS(0), mInputLayout(0), mGridIndexCount(0),
	mTheta(1.5f*MathHelper::Pi), mPhi(0.1f*MathHelper::Pi), mRadius(200.0f)
{
	mMainWndCaption = L"Light Demo";
...
void LightingApp::DrawScene()
{
	...
	SetShaderParameters(mWavesWorld);
...
float LightingApp::GetHeight(float x, float z)const
{
...
void LightingApp::BuildFX()
{
	...
	compilationMsgs = 0;
	hr = D3DX11CompileFromFile(L".\\shader\\Light.hlsl", 0, 0, "PS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS,
...
void LightingApp::SetShaderParameters(const XMFLOAT4X4 worldMatrix)
{
