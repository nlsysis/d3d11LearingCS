...
NeighborTable::~NeighborTable()
{
	SAFE_DELETE(m_pointExtraData);
	SAFE_DELETE(m_neighborDataBuf);
}
...
void NeighborTable::point_commit(void)
{
	...
	{
		growDataBuf(m_dataBufOffset + index_size + distance_size);
	}
...
void NeighborTable::growDataBuf(UINT need_size)
{
