...
float3 CalcGridPos(float3 position, float3 originPosW,float3 cellSize)
{
    int3 gridPos;
...
void BuildGridCS(uint3 Gid : SV_GroupID, uint3 DTid : SV_DispatchThreadID, uint3 GTid : SV_GroupThreadID, uint GI : SV_GroupIndex)
{
    ...
    int3 gridPos = CalcGridPos(position, originPosW, g_vGridDim.xyz);
    uint gridHash = CalcGridHash(gridPos, g_vGridSize.xyz);
...
void DensityCS_Grid(uint3 Gid : SV_GroupID, uint3 DTid : SV_DispatchThreadID, uint3 GTid : SV_GroupThreadID, uint GI : SV_GroupIndex)
{
    ...
    int3 gridPos = CalcGridPos(P_position, originposW, g_vGridDim.xyz);
    uint gridHash = CalcGridHash(gridPos, g_vGridSize.xyz);
    for (int z = -1; z <= 1; ++z)
    {
        for (int y = -1; y <= 1; ++y)
        {
            for (int x = -1; x <= 1; ++x)
            {
                ...
                uint N_gridHash = CalcGridHash(N_position, g_vGridSize.xyz);
